...
int encodeTeacher(Teacher * p, char ** outData, int * outlen)
{
	...
	/*pnext = phead->next;
	DER_ItAsn1_WriteInteger(p->age, &pnext);*/

	/*为什么encode不可以pnext=phead->next,而decode两种方式都可以*/

	// 3. 编码p
	pnext = phead->next;
	EncodeChar(p->p, strlen(p->p)+1, &pnext->next);
	/*pnext = pnext->next;
	EncodeChar(p->p, strlen(p->p) + 1, &pnext);*/
	...
	/*pnext = pnext->next;
	DER_ItAsn1_WriteInteger(p->plen, &pnext);*/
...
int decodeTeacher(char * inData, int inLen, Teacher ** p)
{
	...
	DER_ItAsn1_ReadInteger(phead->next, &teacher->age);
	...
	// 4. plen
	pnext = pnext->next;
	DER_ItAsn1_ReadInteger(pnext->next, &teacher->plen);
