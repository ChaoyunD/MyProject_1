...
public:
    ...
    GameLevel gamelevel;//人机难度

    void startGame(GameType type,GameLevel level); // 开始游戏
    void calculateScore(GameLevel level); // 计算评分
    void actionByPerson(int row, int col); // 人执行下棋
    void actionByAI(int &clickRow, int &clickCol,GameLevel level); // 机器执行下棋
